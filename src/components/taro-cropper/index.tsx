...
class TaroCropperComponent extends Taro.PureComponent<TaroCropperComponentProps, TaroCropperComponentState> {
  ...
  update() {
    ...
    // 绘制半透明层
    this.cropperCanvasContext.beginPath();
    // TAG leon 修改半透明层颜色加深 2020-3-10
    easySetFillStyle(this.systemInfo, this.cropperCanvasContext, 'rgba(0, 0, 0, 0.5)');
  ...
  _getNewScale(oldScale: number, oldDistance: number, touch0: CanvasTouch, touch1: CanvasTouch) {
    ...
    // TAG leon 修改缩放速度 2020-3-10
    return oldScale + 0.01 * (newDistance - oldDistance);
  ...
  // TAG leon 增加双手指锁定，修复在双手指缩放时，放开一指，坐标中心跳跃问题 2020-3-10
  towTouch: false
  ...
  handleOnTouchMove(e: CanvasTouchEvent) {
    ...
    // 单指手势触发
    if (!this.towTouch && e.touches.length === 1) {
